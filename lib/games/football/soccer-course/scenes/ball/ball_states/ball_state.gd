...
func process_gravity(delta: float, bounciness: float = 0.0) -> void:
	# Phase 8: height (like position) is replicated directly from the host
	# via MultiplayerSynchronizer - a non-authority client running this too
	# would fight the incoming replicated value every frame.
...
func move_and_bounce(delta: float) -> void:
	# Same reasoning as process_gravity above: position is the client-side
	# interpolation target (Ball.network_target_position/_process), so a
	# non-authority client must not also drive position via its own local
	# physics here.
	if NetworkManager.is_networked() and not ball.is_multiplayer_authority():
		return

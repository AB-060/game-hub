...
func refresh_bracket_stage(stage: Tournament.Stage) -> void:
	...
	if stage < Tournament.Stage.COMPLETE:
		...
		_show_champion_banner(flag_nodes[0])

# Small, honest addition on top of the already-real champion moment (winner-
# label texture + winner's flag + MusicPlayer.WIN already played in _ready):
# a "CHAMPION: <country>" text label with a scale-in flourish, same
# Tween-ownership pattern as ui.gd's _animate_goal_text (created from the
# longest-lived relevant node here - the label itself - and only created
# once since refresh_bracket_stage(COMPLETE) only runs once per screen
# instance). No new assets/scenes - built in code, parented under the same
# flag node's parent so it lays out sensibly next to the winner flag.
var _champion_label : Label = null

func _show_champion_banner(winner_flag_node: BracketFlag) -> void:
	if _champion_label != null:
		return
	_champion_label = Label.new()
	_champion_label.text = "CHAMPION: %s" % tournament.winner
	_champion_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	_champion_label.add_theme_font_size_override("font_size", 12)
	var parent_container := winner_flag_node.get_parent()
	if parent_container != null:
		parent_container.add_child(_champion_label)
	else:
		add_child(_champion_label)
	_champion_label.scale = Vector2(1.8, 1.8)
	_champion_label.pivot_offset = _champion_label.size * 0.5
	var tween := _champion_label.create_tween()
	tween.tween_property(_champion_label, "scale", Vector2.ONE, 0.4).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)

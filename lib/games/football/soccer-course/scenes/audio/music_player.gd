...
func _ready() -> void:
	...
	# Settings (an earlier autoload) already created the "Music" bus by the
	# time this runs; routed here so Settings.set_music_volume()/
	# set_music_enabled() control it independently of SFX/master.
	bus = "Music"
